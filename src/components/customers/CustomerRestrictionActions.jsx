...
import {
  ...
  MenuItem, Stack, TextField, Typography, Divider, Skeleton,alpha
} from '@mui/material';
...
const activeBan = activeRestrictions.find((r) => r.type === 'ban') || null;
...
<Stack direction="row" spacing={1.2} flexWrap="wrap" sx={{ rowGap: 1.2, mb: 2.5 }}>
  <Button
    startIcon={<ReportGmailerrorredRoundedIcon />} variant="outlined" color="warning"
    onClick={() => setOpenAction({ type: 'warning' })}
  >
    Warn
  </Button>
  <Button
    startIcon={<SpeedRoundedIcon />} variant="outlined" color="info"
    disabled={isBanned}
    onClick={() => setOpenAction({ type: 'limit' })}
  >
    Limit
  ...
  <Button
    startIcon={<PauseCircleOutlineRoundedIcon />} variant="outlined" color="secondary"
    disabled={isBanned}
    onClick={() => setOpenAction({ type: 'suspend' })}
  >
    Suspend
  ...
  <Button
    startIcon={<BlockRoundedIcon />} variant="outlined" color="error"
    onClick={() => setOpenAction({ type: 'ban' })}
  >
    Ban
  </Button>
...
{isBanned && (
  <Alert severity="error" sx={{ mb: 2.5 }}>
    This account is currently banned. Suspend and Limit are disabled while the ban is active.
  </Alert>
)}
      {loading ? (
        <Stack spacing={1}>
          ...
          {history.length > 0 && (
            <Stack spacing={1.2}>
              ...
              {history.map((r) => {
                ...
                return (
                  <Box key={r.id} sx={{ p: 1.5, borderRadius: 1, bgcolor: 'action.hover' }}>
